...
// --- پێکهاتەی هەڵبژاردنی بەروار بە سکرۆڵ و کلیک ---
    ...
    <div>
      ...
      >
        <CalendarOutlined 
          ...
          style={{ color: "#0d9488", marginLeft: "8px", fontSize: "16px", cursor: 'pointer' }} 
        />
...
export const DrawList: React.FC = () => {
  ...
  // --- لێرەدا هەردووکیان گۆڕدراون بۆ بەرواری ئەمڕۆ ---
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let query = supabase.from("draw").select(`*, mshtary(Mname)`)
      ...
      msg.error("کێشەیەک لە بارکردنی زانیارییەکان هەیە");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ padding: "8px", background: "#f8fafc", minHeight: "100vh" }}>
      <Card style={cardStyle} bodyStyle={{ padding: "12px" }}>
        <Row gutter={[12, 12]} align="bottom">
          <Col xs={24} sm={24} md={7}>
            <Text style={{ fontSize: "11px", color: "#64748b", fontWeight: "bold", display: "block" }}>ناوی وەرگر:</Text>
            <Select 
              placeholder="هەموو مشتەرییەکان" 
              allowClear 
              showSearch 
      ...
      <Card 
        ...
        title={
          <Space style={{ color: "#0d9488" }}>
            <TableOutlined /> 
            <span style={{ fontWeight: "bold", fontSize: "14px" }}>لیستی پێدانەکان (Draw)</span>
          </Space>
        }
        bodyStyle={{ padding: 0 }}
      >
            ...
            render={(_, record: any) => (
              <Space size="small">
                <Button 
                  size="small" 
                  icon={<EditOutlined />} 
                  onClick={() => navigate("/draw", { state: { editData: record } })} 
                />
                <Popconfirm 
                  title="سڕینەوە؟" 
                  onConfirm={async () => { 
                    await supabase.from("draw").delete().eq("id", record.id);
                    fetchData();
                    msg.success("سڕایەوە");
                  }}
                  okText="بەڵێ"
